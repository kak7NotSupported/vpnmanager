    ...
    override fun onUpdateReceived(update: Update?) {
        ...
        update?.takeIf { it.hasMessage() && it.message.hasText() }?.let {
            ...
            val sendMessage = when (messageText) {
                ...
                "/contact_us" -> MessageBuilder.buildMessage(chatId, "contact_us", KeyboardFactory.KeyboardType.CONTACT_US, update.message.from.id.toString())
                "/referral" -> MessageBuilder.buildMessage(chatId, "referral_text", KeyboardFactory.KeyboardType.REFERRAL)
                "/localereload" -> {
                    LocaleManager.reloadData()
                    null
                }
                "/admincommands" -> MessageBuilder.buildMessage(chatId, "admincommands")
                LocaleManager.getMessage("platforms") -> MessageBuilder.buildMessage(chatId, "platforms_text", KeyboardFactory.KeyboardType.PLATFORMS)
                LocaleManager.getMessage("about_us_button") -> MessageBuilder.buildMessage(chatId, "about_us_text", KeyboardFactory.KeyboardType.ABOUT_US)
                // todo reset password
